...
class _CitasPageState extends State<CitasPage> with SingleTickerProviderStateMixin {
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: _InfoPill(Icons.attach_money, 'Bs. ${c['monto_pagado'] ?? 0}')),
          ]),
          if (estado == 'activo') ...[
            const SizedBox(height: 12),
            Row(children: [
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 12),
          _buildDropdown('✂️ Servicio', servicios, (v) { setS(() { servicioId = v; final srv = servicios.firstWhere((s) => s['id_servicio'] == v, orElse: () => {}); montoCtrl.text = '${srv['precio'] ?? 0}'; }); }, servicioId, 'id_servicio', 'nombre_del_servicio'),
          const SizedBox(height: 12),
          _buildDropdown('👷 Groomer', [{'id_trabajadores': null, 'nombre': 'Sin asignar'}, ...groomers], (v) => setS(() => groomerId = v), groomerId, 'id_trabajadores', 'nombre'),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: PetSpaTextField(controller: fechaCtrl, label: 'Fecha', icon: Icons.calendar_today, keyboardType: TextInputType.datetime)),
            const SizedBox(width: 12),
          ...
          GradientButton(
            ...
            onPressed: () async {
              final res = await ApiService.crearCita({'id_mascota': mascotaId, 'id_servicio': servicioId, 'empleado_acargo': groomerId, 'fecha_cita': fechaCtrl.text, 'monto_pagado': double.tryParse(montoCtrl.text) ?? 0});
              if (!mounted) return;
              Navigator.pop(ctx);
  ...
  void _terminar(int idCita) async {
    // Cargamos cajas para el dropdown
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          GradientButton(
            ...
            onPressed: () async {
              ...
              if (!mounted) return;
              Navigator.pop(ctx);
              if (res['success'] == true) { PetSpaSnack.show(context, 'Cita terminada ✅'); _load(); }
              else PetSpaSnack.show(context, res['message'] ?? 'Error', error: true);
            },
          ),
        ]),
