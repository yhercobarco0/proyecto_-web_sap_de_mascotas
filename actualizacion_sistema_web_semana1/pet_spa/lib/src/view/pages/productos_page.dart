...
class _ProductosPageState extends State<ProductosPage> {
  ...
  double _parsePrecio(dynamic val) {
    if (val == null) return 0.0;
    if (val is num) return val.toDouble();
    return double.tryParse(val.toString()) ?? 0.0;
  }

  int _parseCantidad(dynamic val) {
  ...
  void _addToCart(Map p) {
    ...
    if (idx >= 0) {
      setState(() {
        _carrito[idx]['cantidad'] = _parseCantidad(_carrito[idx]['cantidad']) + 1;
      });
    } else {
      setState(() {
        _carrito.add({
      ...
      floatingActionButton: Column(
        ...
        children: [
          if (_carrito.isNotEmpty) FloatingActionButton.extended(
            ...
            label: Text('Bs. ${_carrito.fold<double>(0.0, (s, i) => s + _parsePrecio(i['precio']) * _parseCantidad(i['cantidad'])).toStringAsFixed(2)}',
  ...
  void _showCarrito() {
    final total = _carrito.fold<double>(0.0, (s, i) => s + _parsePrecio(i['precio']) * _parseCantidad(i['cantidad']));
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          ..._carrito.map((item) => Container(
            ...
            child: Row(children: [
              ...
              Row(children: [
                GestureDetector(onTap: () => setS(() { if (_parseCantidad(item['cantidad']) > 1) item['cantidad'] = _parseCantidad(item['cantidad']) - 1; else _carrito.remove(item); }), child: Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: PetSpaTheme.danger.withOpacity(0.15), borderRadius: BorderRadius.circular(6)), child: const Icon(Icons.remove, size: 14, color: PetSpaTheme.danger))),
                Padding(padding: const EdgeInsets.symmetric(horizontal: 10), child: Text('${_parseCantidad(item['cantidad'])}', style: const TextStyle(color: PetSpaTheme.textPrimary, fontWeight: FontWeight.w700))),
                GestureDetector(onTap: () => setS(() => item['cantidad'] = _parseCantidad(item['cantidad']) + 1), child: Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: PetSpaTheme.success.withOpacity(0.15), borderRadius: BorderRadius.circular(6)), child: const Icon(Icons.add, size: 14, color: PetSpaTheme.success))),
              ]),
              const SizedBox(width: 12),
              Text('Bs. ${(_parsePrecio(item['precio']) * _parseCantidad(item['cantidad'])).toStringAsFixed(2)}', style: const TextStyle(color: PetSpaTheme.tealLight, fontWeight: FontWeight.w700)),
            ]),
          )),
          ...
          GradientButton(
            ...
            onPressed: () async {
              ...
              Navigator.pop(ctx);
              if (res['success'] == true) { 
                PetSpaSnack.show(context, '¡Pedido realizado! 🎉');
                
                // Generar mensaje para WhatsApp
                String mensaje = "🐾 *Nuevo Pedido PetSpa* 🐾\n\n";
                for (var i in _carrito) {
                  mensaje += "- ${i['cantidad']}x ${i['nombre']} (Bs. ${i['precio']})\n";
                }
                mensaje += "\n💰 *Total: Bs. ${total.toStringAsFixed(2)}*\n\n¡Por favor, confirme mi pedido!";
                
                final uri = Uri.parse("https://wa.me/59170000000?text=${Uri.encodeComponent(mensaje)}");
                try {
                  if (await canLaunchUrl(uri)) {
                    await launchUrl(uri, mode: LaunchMode.externalApplication);
                  }
                } catch (e) {
                  // Si falla, al menos el pedido ya está en BD
                }
