...
const getAllEmpleados = async () => {
  const query = `
    SELECT t.id_trabajadores, u.nombre, u.email, u.estado, t.sueldo_mensual, h.nombre_habilidad, 
           t.estado_trabajador, t.turno, t.especialidad, t.telefono
    FROM TRABAJADORES t
...
const updateEmpleado = async (id_trabajadores, updates) => {
  const { nombre, email, sueldo_mensual, id_habilidad, estado_trabajador, turno, especialidad, telefono } = updates;
  const query = `
    UPDATE TRABAJADORES
