...
const createCita = async ({
  ...
  empleado_acargo, monto_pagado, fecha_cita, estado_de_cita
}) => {
  const res = await pool.query(`
    INSERT INTO CITAS (id_mascota, id_servicio, pagado_por_cliente, empleado_acargo,
      monto_pagado, fecha_cita, estado_de_cita, motivo_cancelacion)
    VALUES ($1,$2,$3,$4,$5,$6,$7,'No cancelado') RETURNING *
  `, [id_mascota, id_servicio, pagado_por_cliente, empleado_acargo,
    monto_pagado, fecha_cita, estado_de_cita || 'activo']);
  return res.rows[0];
};

const updateCita = async (id_cita, fields) => {
  const { estado_de_cita, motivo_cancelacion, monto_pagado, empleado_acargo, fecha_cita,
  ...
  const res = await pool.query(`
    UPDATE CITAS SET
      ...
      empleado_acargo = COALESCE($4, empleado_acargo),
      fecha_cita = COALESCE($5, fecha_cita),
      estado_de_mascota = COALESCE($6, estado_de_mascota),
      foto_estado_mascota = COALESCE($7, foto_estado_mascota),
      monto_llevado_a_caja = COALESCE($8, monto_llevado_a_caja),
      conforme_cliente = COALESCE($9, conforme_cliente)
    WHERE id_cita = $10 RETURNING *
  `, [estado_de_cita, motivo_cancelacion, monto_pagado, empleado_acargo,
    fecha_cita, estado_de_mascota, foto_estado_mascota,
