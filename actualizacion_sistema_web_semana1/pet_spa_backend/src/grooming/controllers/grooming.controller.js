...
exports.cerrarFicha = async (req, res) => {
  try {
    ...
    const result = await groomingService.cerrarFicha(id, checklist_items);
...
exports.getLogInsumos = async (req, res) => {
  try {
    const result = await groomingService.getLogInsumos();
    res.json(result);
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
};
...
// TRANSACCIONES - Nuevos endpoints
exports.crearTransaccion = async (req, res) => {
  try {
    const result = await groomingService.crearTransaccion(req.body);
    res.status(result.success ? 201 : 400).json(result);
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
};
...
exports.actualizarTransaccion = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await groomingService.actualizarTransaccion(id, req.body);
    res.status(result.success ? 200 : 404).json(result);
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
};
...
exports.getTransaccion = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await groomingService.getTransaccion(id);
    res.status(result.success ? 200 : 404).json(result);
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
};
