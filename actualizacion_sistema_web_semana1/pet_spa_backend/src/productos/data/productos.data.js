...
const getProductosBajoStock = async (umbral = 5) => {
  const res = await pool.query(
    `SELECT * FROM PRODUCTOS WHERE stok_unidad <= $1 AND estado_de_producto = 'activo' ORDER BY stok_unidad`,
    [umbral]
  );
  return res.rows;
};
...
const createDetallePedido = async ({ id_pedidio_cliente, id_producto, cantidad, precio_unitario, total }) => {
  // Reduce stock
  await pool.query(
  ...
  const res = await pool.query(
    `INSERT INTO DETALLE_PEDIDO_CLIENTE (id_pedidio_cliente, id_producto, cantidad, precio_unitario, total)
     VALUES ($1,$2,$3,$4,$5) RETURNING *`,
    [id_pedidio_cliente, id_producto, cantidad, precio_unitario, total]
  );
  return res.rows[0];
};
